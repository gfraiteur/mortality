...
L'épidémie de COVID19 a provoqué des bouleversements de nos sociétés qui n'ont pas été observés en Europe occidentale depuis la seconde guerre mondiale.
Cependant, il est encore difficile de comprendre à quel point cette épidémie est exceptionnelle en comparaison avec les données historiques de mortalité. 
S'agit-il d'un événement qui se répète une fois par siècle seulement ou une fois tous les dix ans? Cet article tente de faire la lumière sur ces questions.

Lorsqu'on évoque les effets de l'épidémie de COVID19, la Belgique est un pays intéressant à étudier pour plusieurs raisons. Premièrement, elle a
le taux de mortalité attribué au COVID19 le plus élevé au monde. Deuxièmement, Statbel, l'agence gouvernementale belge des statistiques, publie des données sur la
mortalité chaque semaine avec un délai de seulement 3 semaines. Dernier point et non des moindres, c'est le pays originaire de l'auteur.

Notre approche consiste à comparer le nombre de décès en 2020 (pour les semaines 1 à `r death_2020_max_week`) avec le nombre de décès qui seraient _attendus_ en 
fonction de la structure de la population au 1^er^ janvier 2020 et des taux de mortalité habituels. La différence entre les attentes et la réalité
nous donne une métrique nommée _excès de mortalité_ quand elle est positive, ou _déficits de mortalité_ quand elle est négative.

Nous comparons cette métrique avec le nombre de décès attribués au COVID19 tel que rapporté par Sciensano, et avec les données historiques de décès et de mortalité.

À la fin de l'article, nous essayons de répondre à quelques questions controversées sur base de ces données historiques.

Cet article, ainsi que tous les scripts R qui ont été utilisés pour calculer les données, sont hébergés sur GitHub à l'adresse https://github.com/gfraiteur/mortality.
Toutes les données proviennent de sources ouvertes et peuvent être téléchargées gratuitement. Si vous avez une question ou une remarque relative à cet article,
ou si vous avez trouvé un bogue ou une inexactitude, veuillez signaler le problème sur GitHub ou, mieux, soumettre une pull request.

## A propos de l'auteur

Gael Fraiteur est sorti de l'Ecole polytechnique de Louvain en 2001 avec un diplôme ingénieur civil en mathématiques appliquées. Il détient également deux diplômes 
mineurs en philosophie de la même université. Il travaille depuis dans l'industrie informatique. En 2004, il a lancé un projet open source nommé PostSharp. En 2009, 
il a fondé une société pour commercialiser et développer le produit. En tant que directeur et ingénieur principal de PostSharp Technologies, l'auteur partage 
désormais son temps entre la R&D, la gestion et le marketing.
