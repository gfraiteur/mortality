---
title: "An analysis of excess mortality in 2020 in Czechia"
...
source("Czechia.R", local = knitr::knit_global())
...
population_structure %>%
  filter( year >= 2009 & age_group %in% c("85+", "75-84", "65-74")) %>%
...
mortality %>%
  ...
  ggplot( aes( x = age, y = mortality, color = sex) ) +
    geom_line() +
  scale_y_log10()
```
...
pivot_wider(model_correction, names_from = sex, values_from = correction  ) %>% 
  merge( minor_age_group, by = "minor_age_group") %>%
  arrange( minor_age_group_sortable ) %>%
  select( minor_age_group, "F", "M") %>%
...
death_expected_weekly %>%
  ...
  ggplot( aes(x = date, y = excess_death)) +
    geom_line( color = 'cyan3') +
    this_theme() +
    scale_x_date( date_breaks = "1 month", labels = date_format("%m/%Y"), name = "month") +
    ggtitle("Excess deaths per week") 
```
...
excess_death_2020 %>% 
  merge( minor_age_group ) %>%
  arrange( minor_age_group_sortable ) %>%
  filter( minor_age_group_sortable > "30" ) %>%
  select( minor_age_group, sex, expected_mortality, observed_mortality, excess_mortality, excess_death_percent  )   %>% 
...
excess_death_2020 %>%
  ...
  reshape2::melt(id=("age_min")) %>%
  ggplot(aes(x=age_min, y=value, color=variable)) +
#    geom_bar(aes(fill=variable),stat="identity", position=position_dodge())       +  
#  geom_point(aes()) +
  geom_line(aes(color=variable),alpha=0.5) +
  scale_y_log10() +
  geom_smooth(method="lm", se = FALSE, linetype="dashed", size=0.1) +
  this_theme() 
  
excess_death_2020 %>%
  merge( minor_age_group ) %>%
#  filter( minor_age_group_sortable >= "40") %>%
  group_by(minor_age_group_sortable, sex) %>%
  summarise(excess_death_rate = sum(excess_death) / sum(population_count) ) %>%
  ggplot(aes(x=minor_age_group_sortable, y=excess_death_rate, fill = sex)) +
    scale_y_continuous(labels = scales::percent_format(accuracy = 0.1), "excess death rate", minor_breaks = seq(0,0.025,0.001)) +
...
excess_death_2020 %>%
   merge( minor_age_group ) %>%
    filter( minor_age_group_sortable > "30" ) %>%
    ggplot(aes(x=minor_age_group_sortable, y=excess_death_percent, fill=sex)) +
...
mortality_in_2020$mortality_2020 <- mortality_in_2020$intercept_coeff + 2020 * mortality_in_2020$year_coeff
mortality_in_2020 <- mortality_in_2020 %>% select( -year_coeff, -intercept_coeff )
...
compared_mortality_for_graph %>%
  ...
  scale_y_log10(labels = scales::percent_format(accuracy=1), name = "death rate relatively to 2020 normal", breaks = 10 ^ seq(0,2,0.2), limit = c(1,25))  +
  this_theme() +
  ggtitle("Historic death rate relative to the expected death rate in 2020")
```
...
mortality_deviation_min_year <- 1950
...
cum_compared_covid_death %>%
  group_by( date ) %>%
  summarise( cum_covid_death = sum(cum_covid_death), cum_excess_death = sum(cum_excess_death) ) %>%
  ggplot( aes(x = date)) +
    geom_line(aes(y = cum_covid_death, col = "covid deaths")) + 
    geom_line(aes(y = cum_excess_death, col = "excess deaths")) +
    geom_hline(yintercept = 0) + 
    scale_x_date( labels = date_format("%m/%Y"), date_breaks = "1 month", name = "date") +
    scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE), name = "deaths per week", breaks = seq(0,12000,1000), minor_breaks = NULL ) +
    ggtitle("Weekly COVID19 deaths vs excess cumulative deaths") + 
    this_theme() +
    theme(legend.title = element_blank())
```
...
compared_covid_death_by_age_group <-
  compared_covid_death %>%
  filter( minor_age_group >= "45") %>%
                 group_by( minor_age_group ) %>%
                 summarise( covid_death = sum(covid_death), excess_death = sum(excess_death))

compared_covid_death_age_group_85 <- 
    compared_covid_death_by_age_group %>% filter(age_group == "85+")

reshape2::melt(compared_covid_death_by_age_group,id=("minor_age_group")) %>%
  ggplot( aes(x=minor_age_group, y=value, fill=variable)) +
    geom_bar(stat='identity', position='dodge') +
    scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE), name = "deaths", limits = c(0,3000)) +
